...
| `popmax_estimated_gnomad_AF` | For `HGDP_haplotype`: `popmax_fraction_phased × min(gnomAD_AF[component, max_pop])` over the component variants — i.e. the phase ratio scaled onto the haplotype's gnomAD frequency ceiling in `max_pop` (see `estimated_gnomAD_haplotype_AF_{POP}` for why this `min`-over-gnomAD component can differ from `fraction_phased`'s local-rarest anchor). For `gnomAD_variant`: the gnomAD AF in `max_pop`. Equivalent to `estimated_gnomAD_haplotype_AF_{max_pop}`. |
| `popmax_fraction_phased` | For `HGDP_haplotype`: phase ratio, a proportion in `[0, 1]` — the share of chromosomes carrying the rarest-frequency component variant (rarest by local HGDP+1KG AF) that also carry the full haplotype, computed from counts as `popmax_empirical_AC / AC[rarest-frequency component, max_pop]`. `1.0` for `gnomAD_variant`. Equivalent to `fraction_phased_{max_pop}`. |
...
| `fraction_phased_{POP}` | Per-pop phase ratio, a proportion in `[0, 1]`: `empirical_AC_{POP} / AC[rarest-frequency component, POP]` — haplotype carrier count over the carrier count of the rarest-frequency component variant in that pop (rarest by *local* HGDP+1KG AF). Computing it from counts (rather than `empirical_AF / min(local AF)`) keeps it `<= 1` when component call rates (AN) differ. `1.0` for `gnomAD_variant` rows. Uses *that pop's own* counts (not `max_pop`'s). |
...
chrY non-PAR haplotypes come from the unphased chrY release VCF and cover `XY` males only; the genotypes are haploid, so they need no phasing.

> [!IMPORTANT]
> **Sample-input assumption: sex-aneuploid samples must not have an assigned population.**
> The haplotype computation assumes all samples that survive the population filter have either an `XX` or `XY` karyotype, so that the chrX and chrY non-PAR ploidy correction (males treated as haploid) is well-defined. chrY non-PAR is fully haploid and keeps `XY` males only (`XX` females carry no chrY genotypes); the gnomAD 3.1.2 chrY release VCF has no variant records in the pseudoautosomal (PAR) regions, so no chrY variant reaching this step is pseudoautosomal. This invariant is enforced indirectly: samples are kept only when their `gnomad_population_inference.pop` is in the configured set of populations, and the gnomAD 3.1.2 HGDP+1KG sample-meta source declines to assign a population to any sex-aneuploid sample (`X`, `XXY`, `XYY`, `ambiguous`) — their PCA-based pop inference is unreliable on non-diploid sex karyotypes. Aneuploid samples therefore have `pop = None` upstream and get dropped by the population filter before the chrX/chrY correction runs.
>
> If you point the workflow at a different sample-metadata source that *does* assign populations to aneuploid samples, those samples will pass the population filter and their genotypes will be fed into the chrX/chrY non-PAR correction with incorrect ploidy assumptions. The pipeline does not currently check for this; users supplying custom sample metadata are responsible for filtering aneuploid samples upstream.
