...
> [!NOTE]
> If you use [CALITAS](https://github.com/editasmedicine/calitas) for this purpose, the bundle contains a remapping script that can be used in conjunction with the index database to give you output files with coordinates in the GRCh38 reference genome assembly, making the output seamlessly integrate into workflows that already handle CALITAS output on the reference genome.
...
| `min_variant_allele_freq` | 0.005 | Raise for tighter / lower for looser variant inclusion |
| `min_haplotype_allele_freq` | 0.005 | Raise for tighter / lower for looser haplotype inclusion |
...
The [Hail](https://hail.is/) steps are best run serially with `snakemake -j1` as Hail by default will use all available cores, so there is no efficiency gain from snakemake's parallelizing.
...
Among the 1,947,860 haplotypes found by both algorithms:

| | Count | % of shared |
|---|---:|---:|
| Same popmax AC   | 1,741,162 | 89.4% |
| New > old        |   204,086 | 10.5% |
| Old > new        |     2,612 |  0.1% |
...
Second, `popmax` may shift to a different population: when it shifts to a population with more carriers, `popmax_AC` rises (the `new > old` rows); when it shifts to a population that happens to have a smaller AN but a higher empirical AF, the same haplotype's `popmax_AC` can fall (the 2,612 `old > new` rows).
...
#### Original-only haplotypes

| Category | Count |
|----------|-------|
| Proper contiguous sub-fragment of a shared haplotype with identical per-population AC vector | 74,920 |
| Proper contiguous sub-fragment of a new-only haplotype with identical per-population AC vector | 3,969 |
| Not emitted by new algorithm | 175 |

The new algorithm correctly recognizes 78,889 (3.9%) of haplotypes generated by the old algorithm were redundant; their information was entirely contained in a longer haplotype that in most cases was already being emitted.
For the 3,969 haplotypes that only had a longer haplotype from the new algorithm, this is due to the adjusted counting correctly crediting the longer fragment with carriers from all the parent blocks in which it appears.
